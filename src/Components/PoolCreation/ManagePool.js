...
const ManagePool = (props) => {
  ...
  let userEmail =
    useSelector(selectAuthTokenParsed)?.preferred_username || "Unknown User";
  ...
  let handleDeleteAssignedUser = async (user) => {
    ...
    try {
      ...
      if (payload.users_assigned)
        dispatch(setAssignedUsers(payload.users_assigned));
  ...
  const handleRebuildPool = async (vhdPath) => {
    try {
      ...
      toast.success(payload.msg || "Pool rebuilt successfully", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  useEffect(() => {
    ...
    if (selectedPoolDetails?.cluster_id) {
      fetchCluster();
    }
  }, [selectedPoolDetails]);
