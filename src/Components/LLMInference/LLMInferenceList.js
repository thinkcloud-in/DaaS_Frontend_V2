...
const LLMInference = () => {
  ...
  const fetchList = useCallback(
    (page, size) => {
      if (token)
        dispatch(fetchPrivateLLMListThunk({ token, page, pageSize: size }));
    },
    [token, dispatch],
  );
  ...
  const handleSelectAll = (e) => {
    setSelectedPools(
      e.target.checked ? inferencePools.map((item) => item.id) : [],
    );
  };
  ...
  const executeDelete = (snapshot, totpCode = null) => {
    if (snapshot.type === "single") {
      if (selectedPoolForDrawer?.id === snapshot.id)
      ...
      dispatch(deletePrivateLLMThunk({ token, id: snapshot.id, totpCode }))
        ...
        .then(() => {
          toast.success(
            `Pool "${snapshot.name || snapshot.id}" deleted successfully`,
          );
          silentFetch();
        ...
        .catch((err) => {
          toast.error(
            typeof err === "string"
              ? err
              : err?.detail || err?.message || "Failed to delete",
          );
        });
    } else {
      ...
      Promise.all(
        ids.map((id) =>
          dispatch(deletePrivateLLMThunk({ token, id, totpCode })).unwrap(),
        ),
      )
        .then(() => {
    ...
    <div className="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen text-left items-start flex flex-col w-full relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between pb-6 border-b border-gray-200 dark:border-gray-700 mb-6 w-full">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Click on any pool row to navigate to its deployed Virtual Machines
            list page.
          </p>
        </div>
                ...
                <th className="py-3 px-4 w-12 text-center">
                  <input
                    ...
                    checked={
                      ...
                      selectedPools.length === inferencePools.length
                    }
                  />
                </th>
                ...
                inferencePools.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-gray-900 dark:text-gray-100 font-semibold max-w-[160px] truncate">
                        {item.name}
                      </td>
                      ...
                      >
                        <button
                          ...
                          className="p-1 hover:bg-gray-100 dark:bg-gray-700 rounded-full transition-colors inline-flex text-gray-500 dark:text-gray-400"
                        >
      ...
      {activeDropdownId !== null && (
        <div
          ...
          className="w-36 bg-white dark:bg-gray-800 rounded-md shadow-xl ring-1 ring-black ring-opacity-5 border border-gray-100 text-left divide-y divide-gray-100"
        >
          {(() => {
              ...
              <>
                <div className="py-1">
                  <button
                    ...
                    className="flex items-center w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:bg-gray-900/60 gap-2 font-medium"
                  >
      ...
      {selectedPoolForDrawer && (
        <>
          ...
          <div className="fixed top-0 bottom-0 right-0 max-w-xl w-full bg-white dark:bg-gray-800 shadow-2xl z-[100] flex flex-col justify-between border-l border-gray-200 dark:border-gray-700 h-full">
            <div className="overflow-y-auto flex-1 p-6 text-left">
              <div className="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-gray-700 mb-5">
                <div>
                  <span className="text-xs font-bold uppercase tracking-wider text-[#1a365d] dark:text-blue-300 bg-blue-50 border border-blue-100 px-2.5 py-1 rounded">
                    Pool Details
                  </span>
              ...
              <div className="bg-gray-50 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-6 grid grid-cols-2 gap-x-4 gap-y-3 text-xs">
                <div>
                  <span className="text-gray-400 font-semibold block uppercase tracking-wide">
                    Pool ID
                  </span>
                  <span className="text-gray-900 dark:text-gray-100 font-mono font-medium text-sm block mt-0.5">
                    #{selectedPoolForDrawer.id}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400 font-semibold block uppercase tracking-wide">
                  ...
                  <span
                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold mt-0.5 ${getStatusConfig(selectedPoolForDrawer.status).badge}`}
                  >
                ...
                <div>
                  ...
                  <span className="text-gray-900 dark:text-gray-100 font-mono block mt-0.5">
                    {selectedPoolForDrawer.template || "-"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400 font-semibold block uppercase tracking-wide">
                  ...
                  <span className="text-gray-900 dark:text-gray-100 block mt-0.5">
                    {selectedPoolForDrawer.machine_name || "-"}
                  </span>
                </div>
                <div className="border-t border-gray-200 dark:border-gray-700/60 pt-2 col-span-2" />
                <div>
                  ...
                  <span className="text-gray-700 dark:text-gray-300 font-mono font-semibold block mt-0.5">
                    {selectedPoolForDrawer.head_ip || "-"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400 font-semibold block uppercase tracking-wide">
                  ...
                  <span className="text-gray-700 dark:text-gray-300 font-mono block mt-0.5">
                    {selectedPoolForDrawer.ip_addresses?.join(", ") || "-"}
                  </span>
                </div>
                <div className="border-t border-gray-200 dark:border-gray-700/60 pt-2 col-span-2" />
                <div className="col-span-2">
                  ...
                  <span className="text-gray-800 dark:text-gray-100 font-mono block text-[11px] mt-0.5 bg-white dark:bg-gray-800 p-1.5 rounded border border-gray-200 dark:border-gray-700 break-all">
                ...
                <div>
                  ...
                  <span className="text-gray-700 dark:text-gray-300 block mt-0.5">
                    {selectedPoolForDrawer.created_at?.split(".")[0] || "-"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400 font-semibold block uppercase tracking-wide">
                  ...
                  <span className="text-gray-700 dark:text-gray-300 font-mono block mt-0.5">
                    {selectedPoolForDrawer.vmids?.join(", ") || "-"}
                  </span>
                </div>
              </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="border border-gray-200 dark:border-gray-700 rounded-md overflow-hidden bg-white dark:bg-gray-800">
                    <table className="w-full text-left text-xs">
                      <thead>
                        <tr className="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 font-semibold border-b border-gray-200 dark:border-gray-700">
                      ...
                      <tbody className="divide-y divide-gray-100 text-gray-700 dark:text-gray-300">
                        {selectedPoolForDrawer.nodes.map((n, i) => (
                          <tr key={i} className="hover:bg-gray-50 dark:bg-gray-900/60/60">
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-900/60 border-t border-gray-200 dark:border-gray-700 grid grid-cols-4 gap-2">
              {[
                {
                  action: "start",
                  label: "Start",
                ...
                },
                {
                  ...
                  label: "Shutdown",
                  Icon: PowerIcon,
                  color: "text-red-700 hover:bg-red-50 hover:border-red-300",
                ...
                },
                {
                  ...
                  Icon: StopIcon,
                  color:
                    "text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:bg-gray-700 hover:border-gray-400",
                ...
                return (
                  <button
                    ...
                    disabled={isDisabled}
                    className={`flex flex-col items-center justify-center p-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-xs font-semibold gap-1.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed ${color}`}
                  >
