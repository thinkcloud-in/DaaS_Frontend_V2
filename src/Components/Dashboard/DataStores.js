...
const DataStores = () => {
  ...
  const GRAFANA_TOKEN = "sa-grafana_api_key-9a36c8ba-650a-4ab2-b36d-d7c5eaf41432"
  ...
  useEffect(() => {
    axios
      .get(`${GRAFANA_URL}/api/dashboards/uid/vsphereDS`,
        {
        headers: {
  ...
  const handleVCenterChange = (value) => {
    setSelections((prev) => {
      ...
      // Clear cluster selections when vCenter selection changes
      setClusterNames([]);
  ...
  const buildIframeUrl = () => {
    let url = `${DASHBOARD_GRAFANA_URL}/d/vsphereDS/vmware-vsphere-datastore?orgId=1&from=${gc.timeStamp.startDate}&to=${gc.timeStamp.endDate}&theme=light&disableLazyLoad=true&kiosk`;
  ...
  const handleSelectAll = (variableName, values, isChecked) => {
    ...
    if (variableName === "clustername") {
      ...
      if (isChecked) {
        ...
        // Clear cluster selections when selecting all vCenters
        setClusterNames([]);
        setSelections((prev) => ({
          ...prev,
          vcenter: values,
        ...
        setVcenterNames([]);
        // Clear cluster selections when deselecting all vCenters
        setClusterNames([]);
        setSelections((prev) => ({
          ...prev,
          vcenter: [],
