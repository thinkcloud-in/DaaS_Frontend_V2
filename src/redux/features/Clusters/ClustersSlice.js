    ...
    builder
      ...
      .addCase(createClusterThunk.fulfilled, (state, action) => {
        ...
        if (action.payload.cluster) {
          state.clusters.push(action.payload.cluster);
        }

        // Store warning message if it exists
        if (action.payload.warning && action.payload.message) {
          state.warning = action.payload.message; // or show toast directly in component
        }
      })
      ...
      .addCase(fetchEditInfluxdbDetailsThunk.fulfilled, (state, action) => {
        ...
        state.monitoring.monitoringData = action.payload;
        state.monitoring.wasMonitoringIntegrated = !!(
          action.payload && action.payload.monitoring
        );
      })
